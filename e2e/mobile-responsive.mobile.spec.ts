...
// Mobile-responsive smoke (iPhone 17 Pro Max, 430x932 — the `mobile` Playwright
...
// and free of horizontal overflow at 430px (LoginPage hides its marketing panel
...
test.describe('mobile @ 430px', () => {
  test('mobile project renders at a 430px viewport', async ({ page }) => {
    await page.goto('/login')
    const width = await page.evaluate(() => window.innerWidth)
    expect(width).toBe(430)
  })

  test('login route has no horizontal overflow at 430px', async ({ page }) => {
    await page.goto('/login')
