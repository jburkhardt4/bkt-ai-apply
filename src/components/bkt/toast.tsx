...
export function BktToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="bkt-toast-stack" style={{ position: 'fixed', right: 20, bottom: 'calc(20px + var(--tabbar-h) + var(--safe-bottom))', zIndex: 100, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {toasts.map((t) => (
          <div
            key={t.id}
            style={{
